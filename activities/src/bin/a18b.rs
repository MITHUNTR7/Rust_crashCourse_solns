...
fn authorize(
    ...
    let employee = Database::connect()?;
    
    let found_employee = Database::find_employee(&employee, employee_name)?;
    
    let current_access  = Database::get_keycard(&employee, &found_employee)?;
    
    let required_access = ProtectedLocation::required_access_level(&location);
    
    if current_access.access_level >= required_access{
